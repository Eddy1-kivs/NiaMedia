<template>
  <v-container class="rounded-md" :class="$vuetify.breakpoint.mdAndDown ? 'white' : 'mt-6'">
    ...
    <Head>
      ...
      <meta name="description" :content="`${blog.content} nia media - latest hot news and articles ${metaTags}`" />
    </Head>
    <v-row class="rounded-lg">
      <v-col cols="12" md="9">
        <v-card :outlined="$vuetify.breakpoint.mdAndUp" :flat="$vuetify.breakpoint.mdAndDown">
          <v-card-text :class="$vuetify.breakpoint.mdAndDown ? 'px-0' : ''">
            <div class="text-2xl font text-gray-900 pb-6">
              <div :class="$vuetify.breakpoint.mdAndUp ? 'text-4xl' : ''" v-text="blog.title"></div>
              <div class="inline-flex flex-wrap space-x-3">
                <Link v-for="(tag, index) in blog.tags" :key="index" :href="`/tag/${tag.slug}`" class="no-underline">
                <span class="text-sm leading-none text-orange-600" v-text="`#${tag.slug}`"></span>
                </Link>
              </div>
            ...
            <v-img :src="coverImagePath" height="300" class="rounded-sm"></v-img>
            <div class="remove pt-6" v-html="blog.content"></div>
            <v-row class="my-4">
              <v-col cols="12">
                <div class="title">Related Articles</div>
              </v-col>
              <v-col v-for="(blog, index) in related" :key="index" cols="12" sm="6" md="4">
                <v-card>
            ...
            <v-textarea v-model="form.comment" dense :error-messages="errors.comment" outlined
              placeholder="Type here"></v-textarea>
            <v-btn @click="addComment()" elevation="" color="blue" class="white--text">Submit</v-btn>
            <div class="pt-3">
              <div class="title">All Comments</div>
              <div class="grid pt-4" v-for="(comment, key) in blog.comments" :key="key">
            ...
            <div class="grid space-y-4">
              <div class="grid" v-for="(similar, index) in random" :key="index">
                <div class="inline-flex flex-wrap space-x-2">
                  <Link class="no-underline text-orange-600" v-for="(tag, index) in similar.tags"
                    :href="`/tag/${tag.slug}`" :key="index">
                  <span class="orange--text text-xs capitalize" v-text="`#${tag.name}`"></span>
                  </Link>
                </div>
                <div>
                  <Link v-text="similar.title" :href="`/blog/${similar.slug}`" class="no-underline text-gray-700">
                  </Link>
                </div>
                <div class="text-xs text-gray-500">
